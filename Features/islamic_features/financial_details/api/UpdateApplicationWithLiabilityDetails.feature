...
#Story-CAS-197570
Feature: Update Application with Liability Details
...
Scenario Outline: ACAUTOCAS-18024: validate application update service API error for Liability Details as <criteria> for <ApplicationStage> of <ProductType>
    ...
    When user hits Application update Service API with Liability Details as "<criteria>"
    Then application update service API should throw proper error in response
  ...
  #FeatureID-ACAUTOCAS-163
Scenario Outline: ACAUTOCAS-20862: validate application update service API should run successfully with no error for Liability Details as "valid" for <ApplicationStage> of <ProductType>
    Given user picks an application of "<ProductType>" product type as "<ApplicantType>" applicant at stage "<ApplicationStage>" for category "<Category>" with key "<Key>"
    And user reads data from the excel file "<FinancialDetailsWB>" under sheet "<FinancialDetails_liabilityDetails>" and row <FinancialDetails_liabilityDetails_rowNum>
    When user hits Application update Service API with Liability Details as "valid"
