...
Feature: Capturing_Document_Through_Application_Update_Service_Personal_Finance_Shares_Flow

Scenario: ACAUTOCAS-15742: User should be able to disburse the personal finance shares flow successfully with changing the document status through application service
    Given user is on CAS Login Page
    And user logged in CAS with valid username and password present in "LoginDetailsCAS.xlsx" under "LoginData" and 0
    When user creates an application of "IPF" product type as "indiv" applicant at "sourcing" for "shares" with "e2esingle" from web service
    And user opens an application of "IPF" product type as "indiv" applicant at "sourcing" for "shares" with "e2esingle" from application grid
    And user selects document tab
    And user should be able to verify document status in next stage
      | document_end_flow.xlsx | application_update_documents | 0 |
      | document_end_flow.xlsx | application_update_documents | 1 |
      | document_end_flow.xlsx | application_update_documents | 2 |
      | document_end_flow.xlsx | application_update_documents | 3 |
    And user reads data from the excel file "document.xlsx" under sheet "home" and row 0
    And user changes the document status through API
    ...
    And user selects document tab
    And user should be able to verify document status in next stage
      | document_end_flow.xlsx | application_update_documents | 4 |
      | document_end_flow.xlsx | application_update_documents | 5 |
      | document_end_flow.xlsx | application_update_documents | 6 |
      | document_end_flow.xlsx | application_update_documents | 7 |
#    And user selects the document with document status
#      | document_end_flow.xlsx | application_update_documents | 8 |
#    And user submit the documents with wait
    And user reads data from the excel file "document_end_flow.xlsx" under sheet "application_update_documents" and row 8
    ...
    And user uploads image with particular formats for received documents
      | document_end_flow.xlsx | application_update_documents | 9 | txt  |
      | document_end_flow.xlsx | application_update_documents | 9 | zip  |
      | document_end_flow.xlsx | application_update_documents | 9 | docx |
      | document_end_flow.xlsx | application_update_documents | 9 | doc  |
      | document_end_flow.xlsx | application_update_documents | 9 | odt  |
    And user submit the documents with wait
    And user reads data from the excel file "document_end_flow.xlsx" under sheet "application_update_documents" and row 8
    And user selects the document
    ...
    And user selects document tab
    And user reads data from the excel file "document_end_flow.xlsx" under sheet "application_update_documents" and row 10
    And user selects the document
...
#    And user switch and close next tab
    ...
    And user should be able to verify document status in next stage
      | document_end_flow.xlsx | application_update_documents | 9 |
    And user opens an application present in context from sent to ops grid
