...
Feature: Capturing_Document_Through_Application_Update_Service_Auto_Finance_Flow

Scenario: ACAUTOCAS-15695: User should be able to disburse the auto finance successfully with changing the document status through application service
    Given user is on CAS Login Page
    And user logged in CAS with valid username and password present in "LoginDetailsCAS.xlsx" under "LoginData" and 0
    When user creates an application of "IAF" product type as "indiv" applicant at "sourcing" for "" with "e2esingle" from web service
    And user opens an application of "IAF" product type as "indiv" applicant at "sourcing" for "" with "e2esingle" from application grid
