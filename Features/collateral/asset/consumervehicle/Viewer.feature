...
Feature: Consumer Vehicle Viewer

#  Pre-Requisites---
#  In this feature we will view a Consumer Vehicle attached to the loan application.

  Background:
    Given user is on CAS Login Page
    And user logged in CAS with valid username and password present in "LoginDetailsCAS.xlsx" under "LoginData" and 0

  Scenario Outline: ACAUTOCAS-1231: <CollateralSubType> data reconcile to validate if data was saved properly for <ProductType> at <ApplicationStage> stage
    ...
    Examples:
      | CollateralWB    | Collateral_default | Collateral_default_rowNum | Collateral_vehicleDetails | Collateral_vehicleDetails_rowNum | Collateral_invoiceDetails | Collateral_invoiceDetails_rowNum | ProductType   | ApplicantType   | ApplicationStage   | Category | Key |
      | collateral.xlsx | default            | 620                       | vehicle_details           | 256                              | invoice_details           | 120                              | <ProductType> | <ApplicantType> | <ApplicationStage> |          |     |
    ...
    Examples:
      | CollateralWB    | Collateral_default | Collateral_default_rowNum | Collateral_vehicleDetails | Collateral_vehicleDetails_rowNum | Collateral_invoiceDetails | Collateral_invoiceDetails_rowNum | ProductType   | ApplicantType   | ApplicationStage   | Category   | Key |
      | collateral.xlsx | default            | 620                       | vehicle_details           | 256                              | invoice_details           | 120                              | <ProductType> | <ApplicantType> | <ApplicationStage> | <Category> |     |
  ...
  Scenario Outline: ACAUTOCAS-1232:  CollateralPage should not allow to modify data of <CollateralSubType> Collateral in viewer mode for <ProductType> at <ApplicationStage> stage
    And user opens an "<ProductType>" application for "<ApplicantType>" applicant at "<ApplicationStage>" for "<Category>" with "<Key>" from application grid
    And user opens Collateral Page for adding new collateral
    And user reads data from the excel file "<CollateralWB>" under sheet "<Collateral_default>" and row <Collateral_default_rowNum>
    And user selects collateral type along with collateral sub type
