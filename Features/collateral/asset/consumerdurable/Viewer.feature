...
Feature: Consumer Durable Viewer
...
#  In this feature we will attach an Asset Consumer Durable to the loan application.
  ...
  Scenario Outline: ACAUTOCAS-1147: <CollateralSubType> data reconcile to validate if data was saved properly for <ProductType> at <ApplicationStage> stage
    ...
    Examples:
      ...
      | collateral.xlsx | default            | 621                       | invoice_details           | 122                              | <ProductType> | indiv         | <ApplicationStage> |          |     | <CollateralSubType> |

  Scenario Outline: ACAUTOCAS-1148:  CollateralPage should not allow to modify data of <CollateralSubType> Collateral in viewer mode for <ProductType> at <ApplicationStage> stage
    And user opens an "<ProductType>" application for "<applicantType>" applicant at "<ApplicationStage>" for "<category>" with "<key>" from application grid
    And user opens Collateral Page for adding new collateral
    And user reads data from the excel file "<CollateralWB>" under sheet "<Collateral_default>" and row <Collateral_default_rowNum>
    And user selects collateral type along with collateral sub type
